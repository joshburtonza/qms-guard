...
export function AppSidebar() {
  ...
  return (
    <aside
      className={cn(
        'flex flex-col bg-sidebar text-sidebar-foreground transition-all duration-300 shadow-sidebar',
        collapsed ? 'w-16' : 'w-64'
      )}
    >
      {/* Header */}
      <div className="flex h-16 items-center justify-between px-4 border-b border-sidebar-border">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-bold text-lg leading-none">ASCEND</h1>
              <p className="text-xs text-sidebar-foreground/70">QMS Platform</p>
            </div>
          </div>
        )}
        {collapsed && <Shield className="h-8 w-8 text-sidebar-primary mx-auto" />}
        <Button
          variant="ghost"
          size="icon"
        ...
        {mainNavItems.map((item) => {
          ...
          const linkContent = (
            <Link
              to={item.href}
              className={cn(
                'flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors touch-target',
                active
